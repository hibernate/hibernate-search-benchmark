    ...
    stages {
        stage('Initialize') {
            steps {
                sh '''
                    ...
                    echo "ES_AWS_78_ENDPOINT = ${ES_AWS_52_ENDPOINT}"
        ...
        stage('Build') {
            steps {
                sh """ \
					mvn clean install \
					-U -pl jmh-elasticsearch -am -P search5 \
        ...
        stage('Performance test') {
            steps {
                lock(resource: 'es-aws-78') {
					...
					java \
					-jar benchmarks.jar \
					-jvmArgsAppend -Dhosts=vpc-ci-elasticsearch-52-r5-membawptnueqlpmzkr677lwzoa.us-east-1.es.amazonaws.com \
